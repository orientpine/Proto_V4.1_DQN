...
isTrain = True # 양방향용
isCom = False
scaler = 10 # default = 1 for 기본 10배 xml, 100배 xml을 쓰려면 10 
...
class larva_ver4Env(gym.Env):
    ...
    def __init__(self, render=False):
        ...
        self.observation_space = spaces.Box(np.array([ -50*scaler , -50*scaler, -1.5708, -1.5708, -1.5708, -1.5708, -1.5708, -50, False]),
                                            np.array([  50*scaler ,  50*scaler,  1.5708,  1.5708,  1.5708,  1.5708,  1.5708,  50, True]))# 속도, 변위, 스위치ON/OFF
    ...
    def _reset(self):
        ...
        self.swamId = p.loadURDF(os.path.join(
            path, "larva_model_protov4_100.xml"), headStartPos, headStartOrientation) #larva_model_protov4.xml 는 10배짜리 모델
    ...
    def _assign_throttle(self, action_20):
        ...
        for setNumber in range(0,5):
            p.setJointMotorControlArray(bodyUniqueId=self.swamId,
                                        ...
                                        positionGains=[0.45 for _ in range(0,6)],
                                        velocityGains=[0.5 for _ in range(0,6)],
                                        forces=[0.0000000005 for _ in range(0,6)]
                                        )
    ...
    def _compute_done(self):
        ...
        if self.Forward == True:
            ...
            if self.last_pos_head > 1*scaler or self._envStepCounter%15000 == 15000-1 or  headPos[2] > 0.3*scaler: 
                print(f'\n전진 reward 총합: {self.reward_sum}, 최고 속도: {self.Max_reward}')
                return True
            
        elif self.Forward  == False:
            # tail의 위치가 -1 이상
            if self.last_pos_tail< -1*scaler or self._envStepCounter%15000 == 15000-1 or headPos[2] > 0.3*scaler: 
